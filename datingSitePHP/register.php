        ...
        if (!IsVariableIsSetOrEmpty($email) && !IsVariableIsSetOrEmpty($firstName) && !IsVariableIsSetOrEmpty($lastName) && !IsVariableIsSetOrEmpty($password) && !IsVariableIsSetOrEmpty($dateOfBirth) && !IsVariableIsSetOrEmpty($gender)) {
            if (!IsVariableIsSetOrEmpty($_FILES['fileUpload'])) {
                ...
                if (empty($errors) == true) {
                    ...
                    $image_uploaded = true;
                    try {
                        $insertQueryForRegister = "INSERT INTO mbloemendaal_datingdb.profile(email,password,firstName,lastName,city,birthDate,gender,imgUrl,user_role) values('$email','$password','$firstName','$lastName','$city','$dateOfBirth','$gender','$imageURL','regular')";
